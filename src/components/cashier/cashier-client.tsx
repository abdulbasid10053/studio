...
import { parseVoiceOrder } from "@/app/cashier/actions";
...
export function CashierClient({ menu }: CashierClientProps) {
  ...
  const addToOrder = (item: MenuItem, categoryName: string, options: { level?: string; temp?: string; }, quantity: number = 1) => {
    ...
    setOrder((currentOrder) => {
      ...
      if (existingItem) {
        return currentOrder.map((i) =>
          i.name === finalName ? { ...i, quantity: i.quantity + quantity } : i
        );
      }
      return [...currentOrder, { ...item, name: finalName, quantity: quantity, options }];
    });
  };

  // Voice order handling is now delegated to the VoiceInput component.
  // The onResult callback processes the transcript similarly to the previous startListening implementation.
  const handleVoiceResult = async (transcript: string) => {
    console.log("Voice transcript:", transcript);
    setIsProcessing(true);
    try {
      const items = await parseVoiceOrder(transcript);
      if (items && items.length > 0) {
        items.forEach(item => {
          const category = menu.find(c => c.category === item.category);
          if (category) {
            const menuItem = category.items.find(i => i.name === item.name);
            if (menuItem) {
              addToOrder(menuItem, item.category, item.options || {}, item.quantity);
            }
          }
        });
      }
      ...
      alert("Gagal memproses pesanan suara.");
    } finally {
